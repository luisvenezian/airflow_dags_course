...
with DAG('bigquery_data_load', schedule_interval='@hourly', catchup=False, default_args= default_arguments) as dag:
    load_data = GoogleCloudStorageToBigQueryOperator(
        task_id = 'load_data',
        bucket='lfav-bucket-stage',
        source_objects=['*'],
        source_format='CSV',
        skip_leading_rows=1,
        field_delimiter=',',
        destination_project_dataset_table='lfav-apache-airflow-pipeline.lfavapacheairflowpipeline.history',
    ...
    FROM (
        SELECT
            *,
            ...
            FROM `lfav-apache-airflow-pipeline.lfavapacheairflowpipeline.history`
    ) AS latest
    WHERE rank = 1;
    '''
    ...
    create_table = BigQueryOperator(
        ...
        sql=query,
        destination_dataset_table='lfav-apache-airflow-pipeline.lfavapacheairflowpipeline.latest',
