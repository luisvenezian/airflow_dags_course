...
def move_objects(source_bucket = None, destination_bucket = None, prefix = None, **kwargs):
    storage_objects = kwargs["ti"].xcom_pull(task_ids="list_files") # Resgata o retorno do list_objects
    hook = GoogleCloudStorageHook()
...
with DAG(
    ...
    load_data = GoogleCloudStorageToBigQueryOperator(
        ...
        skip_leading_rows = 1,
        field_delimiter = ",",
        destination_project_dataset_table = "{{ project }}.lfavapacheairflowpipeline.history",
    ...
    FROM (
        SELECT
            *,
            ...
            FROM `{{ project }}.lfavapacheairflowpipeline.history`
    ) AS latest
    WHERE rank = 1;
    """
    ...
    create_table = BigQueryOperator(
        ...
        sql = query,
        destination_dataset_table = "{{ project }}.lfavapacheairflowpipeline.latest",
